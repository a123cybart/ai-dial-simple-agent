...
class DialClient:
    def __init__(
        ...
        self.__tools_dict = {tool.name: tool for tool in tools} if tools else {}
        # 5. Prepare tools list with tool schemas
        self.__tools_schemas = [tool.schema for tool in tools] if tools else []
        # 6. Optional: print endpoint and tools schemas
        print(f"Endpoint: {self.__endpoint}")
        print(f"Tools schemas: {self.__tools_schemas}")
